...
welm_img = open('images/welcome.png', 'rb').read()  
header_img = open('images/header.png', 'rb').read()
aboutus_img = open('images/about.png', 'rb').read()
...
class Menu:
  ...
  def about_us(self):
    ...
    with use_scope("main", clear=True):
        ...
        put_html("<br>")
        put_html("<br>")
        put_image(aboutus_img, width='150%', height='460px')
        data = input_group("Press back button to return to Home Page",[actions('', [ {'label': 'Back', 'value': 1},], name='action', help_text=None),])
    clear('ROOT')
  ...
  def login(self):
    ...
    actions('', [ 
        {'label': 'Student Sign Up', 'value': 1}, 
        {'label': 'Student Login', 'value': 2}, 
        {'label': 'Admin Login', 'value': 3}, 
    ...
    # store who is the current user in class var- user.   
        ...
        if user_inp==2 :
            ...
            if(is_correct_pswd==1):  # name and password match
                with use_scope("main"):
                    put_success(f"\n Welcome, {name_surname.capitalize()}!")
                time.sleep(2)
                return
            else:
  ...
  def menu_for_student(self):
    stud_menu_img = open('images/student_menu.png', 'rb').read()
    ...
    while(choice!=8): # breaks out of loop when 7 i.e. logout is selected
        if mydata.flag==0: 
            ...
            with use_scope("main", clear=True):
                ...
                put_html("<br>")
                put_html("<br>")
                put_image(stud_menu_img,width='100%',height='380px')
  ...
  def menu_for_admin(self):
    clear('ROOT') 
    admin_menu_img = open('images/admin_menu.png', 'rb').read()
    mymachine.flag=0
    choice= None 
    while(choice!=7):  # breaks out of loop when 7 i.e. logout is selected
        ...
        if mymachine.flag==0:
            with use_scope("main", clear=True):
                    ...
                    put_html("<br>")
                    put_html("<br>")
                    put_image(admin_menu_img,width='100%',height='350px')
